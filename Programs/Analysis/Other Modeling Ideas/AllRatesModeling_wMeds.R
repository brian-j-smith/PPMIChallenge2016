...
visit_musts <- 'V04' # Patients must have had this visit
...
for(outVar in rev(outVars)) {
  ...
  i <- i + 1
  tempRatesFits_wMeds <- Fit
  save(tempRatesFits_wMeds, file = 'Programs/Analysis/Models/tempRatesFitswMeds.RData')
  
}

Fit
AllRatesFits_wMeds <- Fit
save(AllRatesFits_wMeds, file = 'Programs/Analysis/Models/AllRatesFits_wMeds.RData')
